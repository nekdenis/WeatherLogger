package com.github.nekdenis.weatherlogger.core.db
...
interface KeyValueRepo<Key : Any, Value : Any> {
     ...
     *
     * [subkey] can be provided to form path: subkey/key/value
     *
     */
    fun save(key: Key, value: Value, subkey: String = ""): Single<Value>
    ...
    /**
     * collect all changes for each key in one list
     */
    @Suppress("UNCHECKED_CAST")
    fun observe(keys: List<Key>): Observable<List<Optional<Value>>>
            = Observable.combineLatest(keys.map { observe(it) }) { it.map { it as Optional<Value> } }
